...
;    Ячейки между заголовком блока и указанным подблоком предварительно
;    очищаются от прежних данных (зануляются) на случай вызова сборщика мусора.
; 2. Создание инфиксных блоков с размещением указателей на них на стеке.
; Поскольку в аккумуляторе может содержаться аргумент, являющийся ссылкой,
; сохраняем его на стеке перед формированием заголовка.
; Для обеспечения корректировки скопированных в тело блока ссылок, сохраняем
; адрес блока (после создания заголовка) на стеке.
CLOSUREREC_impl:
	mov	eax, [opcode.1]	; Количество функторов.
	mov	esi, [opcode.2]	; Количество аргументов замыкания.
	test	esi, esi
;		следом идут смещения для vm_pc (nfuncs шт.); вычисленный адрес
;		переносится в хранилище, за ним аргументы (снимаются со стека)
	lea	ecx, [2*rax-1]	; Размер подблока инфиксов (в словах).
	lea	eax, [2*rax+rsi-1]	; Общий размер блока.
...
;	Сохраняем адрес блока для корректировки скопированных ссылок сборщиком мусора.
	push	alloc_small_ptr
;	Очищаем ячейки инфиксов.
	zero	eax
rep	stos	qword[alloc_small_ptr]
;	Копируем аргементы.
	mov	ecx, esi
	lea	rsi, [rsp + sizeof value]	; пропускаем alloc_small_ptr
rep	movs	qword[alloc_small_ptr], [rsi]
	pop	accu	; адрес блока (за заголовком) скорректирован сборщиком мусора
	mov	rsp, rsi
